...
let to_rad angle = int_of_float (Float.pi *. 180. *. float_of_int angle)
...
let move_turtle turtle length =
  ...
  let dx = length * int_of_float (cos (float_of_int rad)) in
  let dy = length * int_of_float (sin (float_of_int rad)) in
...
let backward turtle l = move_turtle turtle ~-l
